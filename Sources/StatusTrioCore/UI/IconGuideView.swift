...
/// Only an invitation is automatic; expanding the guide is always user initiated.
struct FirstUseIconGuide: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                ...
                .accessibilityLabel(localization.string(.guideDismiss))
                .help(localization.string(.guideDismiss))
            }
            ...
            if isExpanded {
                ...
                HStack {
                    ...
                    Spacer(minLength: 4)
                    Button(localization.string(.guideDone), action: onDismiss)
                }
                .controlSize(.small)
            }
            Divider()
